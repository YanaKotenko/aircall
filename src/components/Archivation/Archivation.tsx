...
import { CallArchive } from './styles';
...
const Archivation = (props: IProps): ReactElement => {
  const { isArchived, onUnarchive, onArchive } = props;
  ...
  return (
    <CallArchive onClick={(): void => handleArchive(isArchived)}>
      {isArchived ? <ArchivedIcon /> : <ArchiveIcon />}
    </CallArchive>
  );
}
